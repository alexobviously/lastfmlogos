...
class ImageUtil {
     ...
     */
    public static function applyFilter($image,$color,$bbg) {
		include("colors.php");
		...
		else if(in_array($color,$cnames))
			self::addColorFilter($image,$colors[$color],$bbg);
...
//        $t = microtime(true);
        ...
        for($x=0; $x<$width; $x++)
        {
            for($y=0; $y<$height; $y++)
            {
                $oldCol = self::getColorArray($image,$x,$y);
                //Cache conversion
                $newColor = $cacheGray[$oldCol[0]];
                //Previous way (what the cache contains)
...
//        exit();
     ...
     */
    private static function addColorFilter($image,array $color,$bbg=false) {
		$_b = $bbg?"_b":"";
        include("cache_".$color[0]."+".$color[1]."+".$color[2].$_b.".php");
        global $cacheColor;
        $width = imagesx($image);
        $height = imagesy($image);

        $ref = imagecolorresolve($image,self::GRAY_VALUE,self::GRAY_VALUE,self::GRAY_VALUE);
        if(!$bbg) imagecolortransparent($image,$ref);
     ...
     */
    public static function getColorArray($image,$x,$y) {
     ...
     */
    public static function getInvertColor(array $color) {
     ...
     */
    public static function getDarkerColor(array $color,$gray) {
     ...
     *
     */
    public static function changeDarkToSpecColor(array $color,array $newColor) {
        $sum = array_sum($color);
	...
	public static function darkspecinv(array $color,array $newColor)
	{
		$col = self::getInvertColor($color);
		$sum = array_sum($col);
        if($sum > (255-(self::GRAY_VALUE-15))*3) {
            $col[0] = $col[0]*($newColor[0]/255);
            $col[1] = $col[1]*($newColor[1]/255);
            $col[2] = $col[2]*($newColor[2]/255);
        }
        return $col;
	}
}

...
class ImageUtil {
    ...
    public static function applyFilter($image,$color) {
		$c = explode("_",$color);
		if(strlen($c[1])==0) $c[1] = "trans";
		$c[0] = "black";
        if($c[0] == 'black')
		{
			//self::addColorFilter($image,array(40,79,148),"white");
            ...
            self::addColorFilterT($image,array(40,79,148));

        else if($c[0] == 'red')
            self::addColorFilterT($image,array(214,17,2));

        else if($c[0] == 'orange')
            self::addColorFilterT($image,array(222,54,0));

        else  if($c[0] == 'turquoise')
            self::addColorFilterT($image,array(69,168,196));
...
//        exit();
     ...
     */
    private static function addColorFilterT($image,array $color) {
        include("cache_".$color[0]."+".$color[1]."+".$color[2].".php");
        global $cacheColor;
        $width = imagesx($image);
        $height = imagesy($image);
